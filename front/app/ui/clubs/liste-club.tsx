            ...
            {resultatRecherche.clubs.map(
                club =>
                    <div key={club.libelle}
                         className="w-1/2 md:w-1/4 lg:w-1/6 px-2 mt-5">
                        {afficheSousFormeDeLien
                            ?
                            <Link
                                className="font-semibold text-xl md:text-2xl hover:underline hover:text-orange-600"
                                href={`/matchs/${encodeURIComponent(club.libelle)}`}>
                                <img src={formatUrlLogo(club.logo)} alt={"logo " + club.libelle}
                                     className="mx-auto rounded-full shadow-[5px_5px_0] shadow-orange-500 mb-2 h-36 w-36"
                                />
                            ...
                            :
                            <span className="break-words mt-2 font-semibold flex items-center flex-col text-center">
                                <p>{club.libelle.replaceAll("/", " / ")}</p>
                                <img src={formatUrlLogo(club.logo)} alt={"logo " + club.libelle} className="h-36 w-36 self-center"/>
                                </span>
                        }
            ...
            {afficheSousFormeDeLien && resultatRecherche.clubs.length > 1 ? <div className="w-1/2 md:w-1/4 lg:w-1/6 flex flex-col justify-between px-2">
                <Link className="mt-2 flex flex-col items-center text-center font-semibold text-xl md:text-2xl hover:underline hover:text-orange-600"
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1" stroke="currentColor"
                         className="mx-auto rounded-full shadow-[5px_5px_0] shadow-orange-500 mb-10 text-orange-600 pt-10 h-36 w-36">
                        <path strokeLinecap="round" strokeLinejoin="round"
                              d="M18 18.72a9.094 9.094 0 0 0 3.741-.479 3 3 0 0 0-4.682-2.72m.94 3.198.001.031c0 .225-.012.447-.037.666A11.944 11.944 0 0 1 12 21c-2.17 0-4.207-.576-5.963-1.584A6.062 6.062 0 0 1 6 18.719m12 0a5.971 5.971 0 0 0-.941-3.197m0 0A5.995 5.995 0 0 0 12 12.75a5.995 5.995 0 0 0-5.058 2.772m0 0a3 3 0 0 0-4.681 2.72 8.986 8.986 0 0 0 3.74.477m.94-3.197a5.971 5.971 0 0 0-.94 3.197M15 6.75a3 3 0 1 1-6 0 3 3 0 0 1 6 0Zm6 3a2.25 2.25 0 1 1-4.5 0 2.25 2.25 0 0 1 4.5 0Zm-13.5 0a2.25 2.25 0 1 1-4.5 0 2.25 2.25 0 0 1 4.5 0Z"/>
                    </svg>
                    Matchs des {resultatRecherche.clubs.length} clubs
                </Link></div> : null}
